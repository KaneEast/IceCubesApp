...
@Observable class ConversationDetailViewModel {
  ...
  func postMessage() async {
    ...
    do {
      let status: ModelsStatus = try await client.post(endpoint: Statuses.postStatus(json: data))
  ...
  private func appendNewStatus(status: ModelsStatus) {
