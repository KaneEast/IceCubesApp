...
@Observable class ConversationsListViewModel {
  ...
  func favorite(conversation: Conversation) async {
    ...
    do {
      let status: ModelsStatus = try await client.post(endpoint: endpoint)
  ...
  private func updateConversationWithNewLastStatus(conversation: Conversation, newLastStatus: ModelsStatus) {
