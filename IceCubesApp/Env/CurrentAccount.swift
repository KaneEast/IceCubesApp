...
@Observable public class CurrentAccount {
  ...
  public private(set) var lists: [ModelsList] = []
  ...
  public var sortedLists: [ModelsList] {
  ...
  public func createList(title: String) async {
    guard let client else { return }
    do {
      let list: ModelsList = try await client.post(endpoint: Lists.createList(title: title))
      lists.append(list)
    } catch {}
  }

  public func deleteList(list: ModelsList) async {
    guard let client else { return }
